...
const REAL_SECONDS_PER_INGAME_MINUTE = 20;
...
const quests = {
  picnic: {
    ...
    goalText: "Harvest 1 crop for Hana.",
    progress: () => totalCrops(),
    rewards: [{ type: "coins", amount: 6 }, { type: "seed", id: "sprout", amount: 2 }],
    consume: true
  },
  cook: {
    ...
    completed: false,
    required: 2,
    intro: "Could you bring me 2 crops for a warm soup?",
    goalText: "Bring Kumo 2 crops.",
    progress: () => totalCrops(),
    rewards: [{ type: "coins", amount: 8 }, { type: "seed", id: "carrot", amount: 1 }],
    consume: true
  },
  decorate: {
    title: "Cozy Corner",
    ...
    completed: false,
    required: 1,
    intro: "Place a new furniture item in your home.",
    goalText: "Place 1 furniture item in your home.",
    progress: () => Math.max(0, homeFurniture.length - 1),
    rewards: [{ type: "coins", amount: 6 }, { type: "furniture", id: "stool", amount: 1 }],
    consume: false
  },
  merchant: {
    ...
    completed: false,
    required: 2,
    intro: "Sell me 2 crops when you can.",
    goalText: "Sell 2 crops to Momo.",
    progress: () => totalCrops(),
    rewards: [{ type: "coins", amount: 10 }, { type: "seed", id: "berry", amount: 1 }],
    consume: true
  },
  introductions: {
    order: ["Hana", "Kumo", "Sora", "Momo", "Poko"],
    met: {
      Hana: false,
      Kumo: false,
      Sora: false,
...
const furnitureOrder = ["stool", "table", "bed"];
...
const cropOrder = ["sprout", "carrot", "berry"];
let selectedCropId = "sprout";
const seedInventory = {
  sprout: 3,
  carrot: 0,
  berry: 0
...
const buildings = [
  ...
  { x: 12, y: 1, w: 4, h: 3, roof: "#ffc7a5", trim: "#ffe5d4", door: "#f2a36b" },
...
// Ensure front paths lead to house doors (connect to the main path row)
houseDoors.forEach((door) => {
  const c = door.c;
  const targetRow = 7;
  const start = Math.min(door.r + 1, targetRow);
  const end = Math.max(door.r + 1, targetRow);
...
const npcs = [
  ...
  { name: "Kumo", role: "Cook", x: 18, y: 6, home: { x: 18, y: 6 }, followOffset: { x: 0.9, y: 0.3 }, mood: "Fresh food brings energy.", questId: "cook", follow: false, blinkOffset: 3.6, style: {
    fur: "#c8f0ff", outfit: "#9ad8ff", accent: "#6aaee8", ear: "bunny", accessory: "scarf", blush: "#ffb2c8"
  }},
  { name: "Sora", role: "Furniture", x: 22, y: 12, home: { x: 22, y: 12 }, followOffset: { x: -0.6, y: -0.8 }, mood: "A cozy home is a happy heart.", questId: "decorate", follow: false, blinkOffset: 5.1, style: {
    fur: "#d8c7ff", outfit: "#b9b6ff", accent: "#8b88d8", ear: "cat", accessory: "flower", blush: "#ffb2e6"
  }},
  { name: "Momo", role: "Merchant", x: 14, y: 12, home: { x: 14, y: 12 }, followOffset: { x: 0.2, y: 0.6 }, mood: "Fresh crops traded for coins!", questId: "merchant", follow: false, blinkOffset: 4.2, style: {
    fur: "#ffe0a8", outfit: "#ffd87a", accent: "#ffb37e", ear: "bear", accessory: "scarf", blush: "#ff9fc1"
  }},
...
function drawTile(r, c, type) {
  ...
  if (type === "grass" || type === "grassDark") {
    ...
    let grass = palette.grass;
    let grassDark = palette.grassDark;
    let accent = "#9bdc8f";
    let flower = "#ffc8dd";
    let snow = "#f4fbff";
    if (season === "Spring") {
    ...
    ctx.fillStyle = type === "grass" ? grass : grassDark;
    ctx.fillRect(x, y, TILE, TILE);
    const noise = hash(r, c);
    if (season === "Winter" && noise > 0.7) {
      ctx.fillStyle = snow;
...
function drawTree(tree) {
  const x = tree.x * TILE + 6;
  const y = tree.y * TILE + 4;
  ctx.fillStyle = "#6ac596";
  ctx.beginPath();
  ctx.arc(x + 10, y + 8, 12, 0, Math.PI * 2);
  ctx.arc(x + 2, y + 16, 9, 0, Math.PI * 2);
  ctx.arc(x + 18, y + 16, 9, 0, Math.PI * 2);
  ctx.arc(x + 10, y + 20, 10, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "#4fae7d";
  ctx.beginPath();
  ctx.arc(x + 6, y + 10, 5, 0, Math.PI * 2);
  ctx.arc(x + 14, y + 12, 5, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "#b37a54";
  ctx.fillRect(x + 8, y + 22, 6, 10);
  ctx.fillStyle = "#8b5e3c";
  ctx.fillRect(x + 7, y + 26, 8, 6);
}
...
function drawCoin(coin) {
  ...
  const shimmer = 0.6 + 0.4 * Math.sin(state.tick * 3 + coin.r * 0.8 + coin.c * 1.3);
  ctx.fillStyle = `rgba(182, 231, 255, ${0.55 + shimmer * 0.3})`;
  ctx.beginPath();
  ctx.arc(x, y, 12, 0, Math.PI * 2);
  ctx.fill();
  ctx.strokeStyle = `rgba(120, 191, 232, ${0.5 + shimmer * 0.4})`;
  ctx.lineWidth = 2;
  ctx.stroke();
  ctx.fillStyle = shimmer > 0.85 ? "#ffe7a3" : "#ffd87a";
  ctx.beginPath();
  ctx.arc(x, y, 7.5, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = shimmer > 0.85 ? "#ffcf6d" : "#f4b24a";
  ctx.beginPath();
...
function addPurchaseEffects(x, y) {
  for (let i = 0; i < 8; i++) {
...
function drawPurchaseEffects() {
  for (const p of purchaseParticles) {
    const alpha = 1 - p.life / p.ttl;
    ctx.fillStyle = `rgba(255, 231, 163, ${alpha})`;
    ctx.beginPath();
    ctx.arc(p.x, p.y, 2.5, 0, Math.PI * 2);
    ctx.fill();
  }
  for (const t of purchaseTexts) {
    const alpha = 1 - t.life / t.ttl;
    drawPixelText("+1", t.x - 10, t.y - t.life * 20, 2, `rgba(246,230,168,${alpha})`);
  }
}
...
function getInteriorScreenOffset() {
...
function getInteriorScreenRect() {
  const canvasRect = canvas.getBoundingClientRect();
  const scaleX = canvasRect.width / canvas.width;
  const scaleY = canvasRect.height / canvas.height;
  const interiorW = INTERIOR_COLS * TILE * scaleX;
  const interiorH = INTERIOR_ROWS * TILE * scaleY;
  const interiorX = canvasRect.left + (canvasRect.width - interiorW) / 2;
  const interiorY = canvasRect.top + (canvasRect.height - interiorH) / 2;
  return { interiorX, interiorY, interiorW, interiorH, canvasRect };
}

function positionEditUi() {
  if (!editToggle || !inventoryPanel) return;
  const stageRect = canvas.parentElement.getBoundingClientRect();
  const { interiorX, interiorY, interiorW, interiorH, canvasRect } = getInteriorScreenRect();
  const toggleTop = Math.max(canvasRect.top + 6, interiorY - 36);
  const toggleLeft = Math.max(canvasRect.left + 6, interiorX);
  editToggle.style.left = `${toggleLeft - stageRect.left}px`;
  editToggle.style.top = `${toggleTop - stageRect.top}px`;

  const panelWidth = inventoryPanel.offsetWidth || 180;
  let panelLeft = interiorX + interiorW + 12;
  const maxLeft = canvasRect.left + canvasRect.width - panelWidth - 12;
  if (panelLeft > maxLeft) panelLeft = maxLeft;
  const panelTop = Math.min(canvasRect.top + canvasRect.height - 200, interiorY + 8);
  inventoryPanel.style.left = `${panelLeft - stageRect.left}px`;
  inventoryPanel.style.top = `${panelTop - stageRect.top}px`;
}

function screenToHomeTile(x, y) {
  const { interiorX, interiorY } = getInteriorScreenOffset();
  const localX = x - interiorX;
  const localY = y - interiorY;
  const c = Math.floor(localX / TILE) + INTERIOR_OFFSET.c;
  const r = Math.floor(localY / TILE) + INTERIOR_OFFSET.r;
  if (r < INTERIOR_OFFSET.r || c < INTERIOR_OFFSET.c ||
      r >= INTERIOR_OFFSET.r + INTERIOR_ROWS || c >= INTERIOR_OFFSET.c + INTERIOR_COLS) {
    return null;
  }
  return { r, c };
}
...
function findFurnitureAt(r, c) {
  for (let i = homeFurniture.length - 1; i >= 0; i--) {
    const item = homeFurniture[i];
    const def = furnitureCatalog[item.id];
    const size = getFurnitureSize(def, item.rotation);
...
function renderInventoryPanel() {
  ...
  for (const id of furnitureOrder) {
    const def = furnitureCatalog[id];
    const count = furnitureInventory[id] || 0;
    const item = document.createElement("div");
    item.className = `inventory-item${count <= 0 ? " disabled" : ""}`;
    item.dataset.id = id;
    ...
    const temp = { id, r: 0, c: 0, rotation: 0 };
    const size = getFurnitureSize(def, 0);
    const scale = Math.min(1, 40 / (Math.max(size.w, size.h) * TILE));
    const drawW = size.w * TILE * scale;
    const drawH = size.h * TILE * scale;
    const offsetX = (48 - drawW) / 2;
    const offsetY = (48 - drawH) / 2;
    iconCtx.save();
    iconCtx.scale(scale, scale);
    iconCtx.translate(offsetX / scale, offsetY / scale);
    const prevCtx = ctx;
    ctx = iconCtx;
    drawFurnitureItem(temp);
    ctx = prevCtx;
    iconCtx.restore();
...
function draw() {
  ...
  if (currentScene === "village") {
    ...
    for (const coin of groundCoins) drawCoin(coin);
    ...
    if (state.editMode && state.dragItem) {
      const def = furnitureCatalog[state.dragItem];
      if (def && state.dragPos && canPlaceFurnitureAt(state.dragPos.r, state.dragPos.c, def, state.dragRotation)) {
        ctx.save();
        ...
        drawFurnitureItem({
          ...
          r: state.dragPos.r,
          c: state.dragPos.c,
          rotation: state.dragRotation
        });
        ctx.restore();
      }
    } else if (state.inOwnHome && !state.editMode) {
      const previewId = getSelectedFurnitureId();
      const def = furnitureCatalog[previewId];
      if (def && furnitureInventory[previewId] > 0) {
        ...
        if (canPlaceFurnitureAt(target.r, target.c, def, state.furnitureRotation)) {
          ...
          drawFurnitureItem({
            ...
            r: target.r,
            c: target.c,
            rotation: state.furnitureRotation
          });
          ctx.restore();
        }
      }
...
function applyQuestRewards(rewards) {
  ...
  for (const reward of rewards) {
    if (reward.type === "coins") {
      ...
      if (seedInventory[reward.id] != null) {
        seedInventory[reward.id] += reward.amount || 0;
      }
    } else if (reward.type === "furniture") {
      if (furnitureInventory[reward.id] != null) {
        furnitureInventory[reward.id] += reward.amount || 0;
      }
    } else if (reward.type === "crop") {
      if (cropInventory[reward.id] != null) {
        cropInventory[reward.id] += reward.amount || 0;
      }
    }
  }
}
...
function rewardSummary(rewards) {
  ...
  return rewards.map((r) => {
    ...
    if (r.type === "seed") return `${r.amount} ${cropCatalog[r.id].name} seeds`;
    if (r.type === "furniture") return `${r.amount} ${furnitureCatalog[r.id].name}`;
    if (r.type === "crop") return `${r.amount} ${cropCatalog[r.id].name}`;
    return "";
  }).filter(Boolean).join(", ");
}

function handleQuestForNpc(npc) {
  const quest = getQuestForNpc(npc);
  if (!quest) {
    showHint(`${npc.name}: I don't have anything right now.`);
    return;
  }
  ...
  if (!quest.accepted) {
    quest.accepted = true;
    ui.objective.textContent = `${quest.title}: ${quest.goalText}`;
    showHint(`${quest.giver}: ${quest.intro}`);
    return;
  }
  if (!quest.completed) {
    ...
    if (npc.name === "Momo") {
      if (progress >= quest.required) {
        openInteraction(
          "merchantQuestSubmit",
          npc,
          `${quest.giver}: Ready to submit the crops?`,
          ["Submit Crops", "Back"],
          "row"
        );
      } else {
        openInteraction(
          "merchantQuestSubmit",
          npc,
          `${quest.giver}: We still need ${quest.required - progress} crops.`,
          ["Submit Crops", "Back"],
          "row"
        );
      }
    ...
    if (progress >= quest.required) {
      ...
      updateHud();
      ui.objective.textContent = `${quest.title} complete!`;
      const rewardText = rewardSummary(quest.rewards);
      showHint(`${quest.giver}: Thank you!${rewardText ? " +" + rewardText : ""}`);
    } else {
      showHint(`${quest.giver}: We still need ${quest.required - progress}.`);
    }
    return;
  }
  showHint(`${quest.giver}: Thanks again!`);
}
...
function isFurnitureBlockingAt(x, y) {
  if (currentScene !== "home") return false;
  const radius = 0.2;
  for (const item of homeFurniture) {
    const def = furnitureCatalog[item.id];
    if (!def) continue;
    if (item.id !== "bed") continue;
    const size = getFurnitureSize(def, item.rotation);
    const left = item.c;
    const right = item.c + size.w;
    const top = item.r;
    const bottom = item.r + size.h;
...
function isWalkablePos(x, y) {
  const r = Math.floor(y);
  const c = Math.floor(x);
  if (!isWalkable(r, c)) return false;
  if (isHouseBlockingAt(x, y)) return false;
  if (isFurnitureBlockingAt(x, y)) return false;
  return true;
}

function isHouseBlockingAt(x, y) {
  if (currentScene !== "village") return false;
  const radius = 0.45;
  for (const house of buildings) {
    const left = house.x;
    const right = house.x + house.w;
    const top = house.y;
    const bottom = house.y + house.h;
    const doorC = house.x + Math.floor(house.w / 2);
    const doorLeft = doorC - 1.0;
    const doorRight = doorC + 0.8;
    const doorTop = house.y + house.h - 0.6;
    const doorBottom = house.y + house.h + 0.9;
    const inDoorGap = x > doorLeft && x < doorRight && y > doorTop && y < doorBottom;
    if (!inDoorGap && x > left && x < right && y > top && y < bottom) {
      return true;
    }
    const roofLeft = house.x;
    const roofRight = house.x + house.w;
    const roofTop = house.y - 0.7;
    const roofBottom = house.y + 0.2;
    if (x > roofLeft && x < roofRight && y > roofTop && y < roofBottom) {
      return true;
    }
  }
  return false;
...
function canPlaceCoinAt(r, c) {
  if (r < 1 || c < 1 || r >= ROWS - 1 || c >= COLS - 1) return false;
  if (!isWalkable(r, c)) return false;
  if (map[r][c] === "water") return false;
  for (const house of buildings) {
    if (r >= house.y && r < house.y + house.h && c >= house.x && c < house.x + house.w) {
      return false;
    }
  }
  if (getPlotAt(r, c)) return false;
  for (const npc of npcs) {
    if (Math.hypot(npc.x - c, npc.y - r) < 1) return false;
  }
  return true;
}
...
function spawnCoin() {
  ...
  for (let i = 0; i < 40; i++) {
    ...
    if (!canPlaceCoinAt(r, c)) continue;
    groundCoins.push({ r, c, x: c + 0.5, y: r + 0.5, value: 1 + Math.floor(Math.random() * 5) });
    break;
  }
}
...
function updatePlayer(dt) {
  ...
  if (isWalkablePos(nextX, nextY) && !isNpcBlocking(nextX, nextY)) {
    player.x = nextX;
    player.y = nextY;
  }

  if (currentScene === "village" && groundCoins.length) {
    for (let i = groundCoins.length - 1; i >= 0; i--) {
      ...
      if (dist < 0.45) {
        ...
        updateHud();
        showHint(`Picked up ${coin.value} coins.`);
      }
    }
  }
...
function buySeedById(id, optionIndex) {
  const crop = cropCatalog[id];
  if (!crop) return;
  if (state.coins < crop.seedCost) {
    showHint(`${crop.name} seeds cost ${crop.seedCost} coins.`);
  ...
  updateHud();
  showHint(`Bought ${crop.name} seeds.`);
  if (state.interaction && state.interaction.type === "seedShop") {
    const prevIndex = state.dialogueIndex;
    state.dialogueOptions = setOptionsForRole("Farmer");
    state.dialogueIndex = Math.min(prevIndex, state.dialogueOptions.length - 1);
    state.dialogueText = `Hana: Added ${crop.name}.`;
    state.dialogueTimer = 6;
    const rect = state.optionRects[optionIndex];
    const box = state.dialogueBox;
    if (rect) {
      addPurchaseEffects(rect.x + rect.w / 2, rect.y + rect.h / 2);
    } else if (box) {
      addPurchaseEffects(box.x + box.w / 2, box.y + box.h / 2);
    }
    playPurchaseSound();
  }
}
...
function sellCropById(id) {
  const crop = cropCatalog[id];
  if (!crop) return;
  if ((cropInventory[id] || 0) <= 0) {
    showHint(`No ${crop.name} to sell.`);
    return;
  }
  cropInventory[id] -= 1;
  state.coins += crop.sell;
  updateHud();
  showHint(`Sold ${crop.name} for ${crop.sell} coins.`);
  if (state.interaction && state.interaction.type === "sellShop") {
    const prevIndex = state.dialogueIndex;
    state.dialogueOptions = setOptionsForRole("Merchant");
    state.dialogueIndex = Math.min(prevIndex, state.dialogueOptions.length - 1);
  }
}
...
function setOptionsForRole(role) {
  ...
  if (role === "Merchant") {
    const opts = cropOrder.map(id => ({
      label: `${cropCatalog[id].name} x${cropInventory[id] || 0}`,
      price: cropCatalog[id].sell,
      action: "sell",
      id
    }));
    opts.push({ label: "Exit", action: "exit" });
...
function performInteractionSelection() {
  ...
  if (state.interaction.type === "farmerMenu") {
    if (index === 0) {
      handleQuestForNpc({ name: "Hana", questId: "picnic" });
      closeInteraction();
      return;
    }
    if (index === 1) {
  ...
  if (state.interaction.type === "furnitureMenu") {
    if (index === 0) {
      handleQuestForNpc({ name: "Sora", questId: "decorate" });
      closeInteraction();
      return;
    }
    if (index === 1) {
  ...
  if (state.interaction.type === "cookMenu") {
    if (index === 0) {
      handleQuestForNpc({ name: "Kumo", questId: "cook" });
      closeInteraction();
      return;
    }
    if (index === 1) {
      openInteraction("cook", { name: "Kumo" }, "Kumo: Want a warm meal?", setOptionsForRole("Cook"));
      return;
    }
    closeInteraction();
    return;
  }
  if (state.interaction.type === "merchantMenu") {
    if (index === 0) {
      handleQuestForNpc({ name: "Momo", questId: "merchant" });
      return;
    }
    if (index === 1) {
      openInteraction("sellShop", { name: "Momo" }, "Momo: Pick a crop to sell.", setOptionsForRole("Merchant"), "row");
      return;
    }
    closeInteraction();
    return;
  }
  if (state.interaction.type === "merchantQuestSubmit") {
    if (index === 0) {
      const quest = quests.merchant;
      if (quest.accepted && !quest.completed) {
        ...
        if (progress >= quest.required) {
          ...
          updateHud();
          ui.objective.textContent = `${quest.title} complete!`;
          showHint(`${quest.giver}: Thank you!`);
        } else {
          showHint(`${quest.giver}: Not enough crops yet.`);
        }
      }
      closeInteraction();
      return;
    ...
    if (index === 1) {
      openInteraction("merchantMenu", { name: "Momo" }, "Momo: Want to sell crops?", ["Quest", "Sell Crops", "Exit"], "row");
      return;
    }
    closeInteraction();
    return;
  }
  if (state.interaction.type === "sellShop") {
    const option = state.dialogueOptions[index];
    if (option?.action === "sell") {
      sellCropById(option.id);
      return;
    }
    closeInteraction();
    return;
...
function talkToNpc(npc) {
  ...
  if (npc.role === "Furniture") {
    openInteraction("furnitureMenu", npc, `${npc.name}: Hi, how is it going?`, ["Quest", "Furniture Shop", "Exit"], "row");
    return;
  }
  if (npc.role === "Merchant") {
    openInteraction("merchantMenu", npc, `${npc.name}: Want to sell crops?`, ["Quest", "Sell Crops", "Exit"], "row");
    return;
  }
  if (npc.role === "Cook") {
    openInteraction("cookMenu", npc, `${npc.name}: What's on your mind?`, ["Quest", "Warm Meal", "Exit"], "row");
    return;
  }
...
function waterPlot() {
  const target = getPlayerTile();
  const plot = getPlotAt(target.r, target.c);
  if (!plot || !plot.planted) {
    showHint("Stand on a planted crop to water.");
    return;
  }
  if (plot.watered) {
    showHint("Already watered.");
    return;
  }
  plot.watered = true;
...
function plantSeed() {
  ...
  if (seedInventory[selectedCropId] <= 0) {
    const fallback = cropOrder.find(id => (seedInventory[id] || 0) > 0);
    if (fallback) {
      selectedCropId = fallback;
    } else {
...
function updateHud() {
  ...
  const totalSeeds = cropOrder.reduce((sum, id) => sum + (seedInventory[id] || 0), 0);
  ui.bag.innerHTML = `Crops: ${state.bag}<br>Seeds: ${totalSeeds}`;
...
function saveGame() {
  const saveData = {
    ...
    quests: {
      ...
      cook: {
        accepted: quests.cook.accepted,
        completed: quests.cook.completed
      },
      decorate: {
        accepted: quests.decorate.accepted,
        completed: quests.decorate.completed
      },
      merchant: {
        accepted: quests.merchant.accepted,
        completed: quests.merchant.completed
      },
      introductions: quests.introductions
    },
    petAdopted: state.petAdopted,
    merchantQuestSold: state.merchantQuestSold
  };
...
function loadGame() {
  ...
  try {
    ...
    selectedCropId = data.selectedCropId ?? selectedCropId;
    ...
    if (Array.isArray(data.farmPlots)) {
      for (let i = 0; i < farmPlots.length; i++) {
        const saved = data.farmPlots[i];
        if (saved && saved.r === farmPlots[i].r && saved.c === farmPlots[i].c) {
          farmPlots[i] = {
            ...farmPlots[i],
            planted: !!saved.planted,
            watered: !!saved.watered,
            growTime: saved.growTime || 0,
            cropId: saved.cropId || null
          };
        }
      }
    }
    ...
    quests.cook.accepted = data.quests?.cook?.accepted ?? quests.cook.accepted;
    quests.cook.completed = data.quests?.cook?.completed ?? quests.cook.completed;
    quests.decorate.accepted = data.quests?.decorate?.accepted ?? quests.decorate.accepted;
    quests.decorate.completed = data.quests?.decorate?.completed ?? quests.decorate.completed;
    quests.merchant.accepted = data.quests?.merchant?.accepted ?? quests.merchant.accepted;
    quests.merchant.completed = data.quests?.merchant?.completed ?? quests.merchant.completed;
    ...
    state.merchantQuestSold = data.merchantQuestSold ?? state.merchantQuestSold;
  } catch (err) {
...
function changeScene(sceneName, spawn) {
  ...
  if (sceneName === "village") {
    ...
    state.dragItem = null;
    if (inventoryPanel) inventoryPanel.classList.remove("show");
    if (editToggle) editToggle.classList.add("hidden");
  } else {
    buildHomeSolids();
    if (editToggle) editToggle.classList.toggle("hidden", !state.inOwnHome);
    if (state.inOwnHome) positionEditUi();
  }
...
function updateQuestState() {
  const active = [quests.picnic, quests.cook, quests.decorate, quests.merchant]
    .find(q => q.accepted && !q.completed);
  if (!active) return;
  if (active.progress() >= active.required) {
    ui.objective.textContent = `${active.title}: Return to ${active.giver}.`;
  } else {
    ui.objective.textContent = `${active.title}: ${active.goalText}`;
  }
}
...
function buildVillageSolids() {
  ...
  for (const house of buildings) {
    setSolidRect(house.x, house.y, house.w, house.h, true);
    const x = house.x * TILE;
    const y = house.y * TILE;
    const w = house.w * TILE;
    const roofLeft = x - 6;
    const roofRight = x + w + 6;
    const roofTop = y - 8;
    const roofBottom = y + 20;
    const leftC = Math.floor(roofLeft / TILE);
    const rightC = Math.floor((roofRight - 1) / TILE);
    const topR = Math.floor(roofTop / TILE);
    const bottomR = Math.floor((roofBottom - 1) / TILE);
    for (let r = topR; r <= bottomR; r++) {
      for (let c = leftC; c <= rightC; c++) {
        setSolidPoint(r, c, true);
      }
    }
  }
  ...
  for (const bush of decor.bushes) setSolidPoint(bush.r, bush.c, false);
  for (const rock of decor.rocks) setSolidPoint(rock.r, rock.c, false);
  for (const lamp of decor.lanterns) setSolidPoint(lamp.r, lamp.c, false);
...
function gameLoop(timestamp) {
  ...
  if (state.interaction && state.dialogueTimer === 0 && state.dialogueOptions.length === 0) {
    closeInteraction();
  }
  ...
  for (let i = purchaseParticles.length - 1; i >= 0; i--) {
    const p = purchaseParticles[i];
    p.life += dt;
    p.x += p.vx * dt;
    p.y += p.vy * dt;
    if (p.life >= p.ttl) purchaseParticles.splice(i, 1);
  }
  for (let i = purchaseTexts.length - 1; i >= 0; i--) {
    const t = purchaseTexts[i];
    t.life += dt;
    if (t.life >= t.ttl) purchaseTexts.splice(i, 1);
  }

  state.coinSpawnTimer -= dt;
  if (state.coinSpawnTimer <= 0) {
    if (groundCoins.length < 3) {
      spawnCoin();
    }
    state.coinSpawnTimer = 60;
  }
...
window.addEventListener("keydown", (e) => {
  if (state.editMode) {
    if (e.code === "KeyR" && state.dragItem) {
      state.dragRotation = (state.dragRotation + 1) % 2;
      return;
    }
    if (e.code === "KeyX" && state.dragItem) {
      if (!state.dragFromInventory) {
        furnitureInventory[state.dragItem] += 1;
        updateHud();
      }
      state.dragItem = null;
      state.dragPos = null;
  ...
  if (e.code === "KeyW") waterPlot();
  if (!state.editMode && e.code === "KeyF") placeFurniture();
  if (!state.editMode && e.code === "KeyX") pickupFurniture();
  if (!state.editMode && e.code === "KeyR") rotateFurniture();
  if (e.code === "KeyH") {
...
canvas.addEventListener("mousemove", (e) => {
  ...
  if (state.editMode && state.dragItem && currentScene === "home") {
    const tile = screenToHomeTile(state.mouse.x, state.mouse.y);
    state.dragPos = tile;
  }
});

canvas.addEventListener("click", () => {
  if (state.editMode && currentScene === "home") {
    if (state.dragItem) {
      const tile = state.dragPos;
      const def = furnitureCatalog[state.dragItem];
      if (tile && def && canPlaceFurnitureAt(tile.r, tile.c, def, state.dragRotation)) {
        homeFurniture.push({ id: state.dragItem, r: tile.r, c: tile.c, rotation: state.dragRotation });
        if (state.dragFromInventory) {
          furnitureInventory[state.dragItem] -= 1;
        }
        state.dragItem = null;
        ...
        showHint("Can't place there.");
      }
      return;
    } else {
      const tile = screenToHomeTile(state.mouse.x, state.mouse.y);
      if (tile) {
        const hit = findFurnitureAt(tile.r, tile.c);
        if (hit) {
          homeFurniture.splice(hit.index, 1);
          furnitureInventory[hit.item.id] += 1;
          updateHud();
          state.dragItem = hit.item.id;
          state.dragRotation = hit.item.rotation;
          state.dragFromInventory = true;
          state.dragPos = tile;
          showHint("Drag to place. Press R to rotate.");
          return;
        }
      }
    }
...
if (editToggle) {
  editToggle.addEventListener("click", () => {
    if (!state.inOwnHome) return;
    state.editMode = !state.editMode;
    if (inventoryPanel) {
      inventoryPanel.classList.toggle("show", state.editMode);
    }
    if (state.editMode) {
      renderInventoryPanel();
      positionEditUi();
...
if (inventoryGrid) {
  inventoryGrid.addEventListener("mousedown", (e) => {
    ...
    if (!id || (furnitureInventory[id] || 0) <= 0) return;
    state.dragItem = id;
    state.dragRotation = state.furnitureRotation;
    state.dragFromInventory = true;
    state.dragPos = null;
  });
}
...
updateIntroObjective();
if (editToggle) {
  editToggle.classList.toggle("hidden", !state.inOwnHome);
  if (state.inOwnHome) positionEditUi();
}
